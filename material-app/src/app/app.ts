...
const STANDARDIZATION_AREAS: StandardizationArea[] = [
  {
    ...
    reason: 'Header, sidebar, breadcrumbs, and action areas recur throughout enterprise products.'
  },
  {
    area: 'Forms and validation states',
    reason: 'Shared field structure and predictable error handling reduce implementation variance.'
  },
  {
    area: 'Tables and status indicators',
    reason: 'Most corporate modules revolve around filtered lists, approvals, and status communication.'
  },
  {
    area: 'Feedback patterns',
    reason: 'Dialogs, snackbars, and empty states are central to trust and workflow transparency.'
  }
];
...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      <p>
        {{ data.name }} will remain in the mock dataset, but the status changes to inactive so the
        list and detail views update consistently.
      </p>
    </mat-dialog-content>
    <mat-dialog-actions align="end">
      <button type="button" mat-button [mat-dialog-close]="false">Cancel</button>
      <button type="button" mat-flat-button color="warn" [mat-dialog-close]="true">Set inactive</button>
    </mat-dialog-actions>
  `
})
...
export class App implements OnInit {
  ...
  protected readonly displayedColumns = ['name', 'department', 'role', 'status', 'hireDate', 'actions'];
  ...
  protected readonly kpis = computed(() => {
    ...
    return [
      ...
      },
      {
        ...
        note: 'Highlights approval flows and dialog usage'
      },
      {
        label: 'Avg. Workload',
        value: `${Math.round(
  ...
  protected saveEmployee(form: NgForm): void {
    if (form.invalid) {
      this.showSnack('Please complete the required fields before saving.');
      return;
    }
    ...
    if (this.formMode() === 'edit' && payload.id !== null) {
      ...
      this.showSnack('Employee record updated.');
    } else {
      ...
      this.formMode.set('edit');
      this.showSnack('Employee created in the mock dataset.');
    }
  ...
  protected refreshDashboard(): void {
    ...
    this.showSnack('Dashboard widgets refreshed.');
  }

  protected openDeactivateDialog(employee: Employee): void {
    this.dialog
      .open(ConfirmStatusDialogComponent, {
        data: { name: this.employeeName(employee) }
      })
      .afterClosed()
      .subscribe((confirmed) => {
        if (!confirmed) {
        ...
        this.employees.update((employees) =>
          employees.map((item) =>
            item.id === employee.id ? { ...item, status: 'Inactive' } : item
          )
        );
        this.showSnack('Employee status changed to inactive.');
      });
  }

  protected showDemoToast(): void {
    this.showSnack('Angular Material snackbar triggered from the style guide.');
  }
